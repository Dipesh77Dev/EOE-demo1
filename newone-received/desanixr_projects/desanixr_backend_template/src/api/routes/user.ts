...
export default (app: Router) => {
  ...
  //Updating user for update profile.
  route.patch(
    '/profile/:userId',
    celebrate({
      body: Joi.object({
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const { user } = await authServiceInstance.Update(userId, req.body as IUserInputDTO);
  ...
  route.patch(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        const authServiceInstance = Container.get(AuthService);
        const userId: string = req.params.userId;
        const { user } = await authServiceInstance.Update2(userId, req.body as IUserInputDTO);
        console.log('After Update', user);
        // return res.status(201).json({ user });
        return res.json({ user }).status(200);
      } catch (e) {
