...
export default class UserSubscriber {
  ...
  public onUserSignUp({ firstName, email, _id }: Partial<IUser>) {
    ...
    try {
      ...
      // MailService.startSequence('user.welcome', { email, firstName })
    } catch (e) {
  ...
  @On(events.user.updateUser)
  public onUserUpdate({ firstName, email, _id }: Partial<IUser>) {
    const Logger: Logger = Container.get('logger');

    try {
    } catch (e) {
      Logger.error(`🔥 Error on event ${events.user.updateUser}: %o`, e);
      throw e;
    }
  }
}
