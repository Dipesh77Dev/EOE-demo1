...
import { screenGenerator, userGenerator, sceneGenerator } from './data-generators';
...
export const createDiscussion = async (discussionProperties?: any) => {
  const discussion = screenGenerator(discussionProperties);
  const res = await db.discussion.create(discussion);
  return res;
};

export const createScreen = async (screenProperties?: any) => {
  const screen = screenGenerator(screenProperties);
  const res = await db.screen.create(screen);
  return res;
};

export const createScene = async (sceneProperties?: any) => {
  const scene = sceneGenerator(sceneProperties);
  const res = await db.scene.create(scene);
  return res;
};
